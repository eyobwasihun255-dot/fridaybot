...
try {
  ...
  for (const demo of selectedDemos) {
    ...
    await new Promise((res) => setTimeout(res, 300)); // tiny delay for Firebase load
  }
...
setTimeout(() => {
  shuffleActive = false;
      ...
      if (this.notifier) {
        this.notifier.send(
          `🕒 Countdown started for room ${roomId} (${durationMs / 1000}s)`
        );
      }
